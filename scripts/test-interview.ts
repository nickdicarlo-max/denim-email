...
function evaluate(
	...
	const foundGeneric = hypothesis.tags.filter((t) =>
		genericTags.includes(t.name),
	);
	checks.push({
	...
	const queryTexts = hypothesis.discoveryQueries.map((q) =>
		q.query.toLowerCase(),
	);
	const entityWords = input.whats.flatMap((w) =>
		w.toLowerCase().split(/\s+/),
	);
	const queriesRefEntities = entityWords.some((word) =>
		queryTexts.some((q) => q.includes(word)),
	);
	checks.push({
	...
	const withAliases = hypothesis.entities.filter(
		(e) => e.aliases.length > 0,
	);
	checks.push({
		name: "Entity aliases generated",
		passed: withAliases.length > 0,
		detail: hypothesis.entities
	...
	const allWhatsPresent = input.whats.every((w) =>
		primaryNames.some(
			(p) => p.includes(w.toLowerCase()) || w.toLowerCase().includes(p),
		),
	);
	checks.push({
	...
	const goalsReflected =
		...
		expectedFields.some((expected) =>
			showOnCardNames.some((actual) => actual.includes(expected)),
		);
	checks.push({
