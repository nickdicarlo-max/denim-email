...
import type { ClusteringConfig, ClusterEmailInput, ClusterCaseInput } from "@denim/types";
...
async function main() {
  ...
  for (const schemaName of [
    "April 11 Test Girls Activities",
    "April 11 Property Management",
  ]) {
    ...
    if (!schema) { log(`\nSchema "${schemaName}" not found`); continue; }
    ...
    const emails = await p.email.findMany({
      ...
      select: {
        ...
        senderDisplayName: true, senderEmail: true,
      },
      orderBy: { date: "asc" },
    });
    ...
    log(`  CREATE: ${creates.length} (${creates.reduce((s, d) => s + d.emailIds.length, 0)} emails)`);
    ...
    if (merges.length > 0) {
      ...
      for (const m of merges.slice(0, 15)) {
        ...
        log(`  score=${m.score.toFixed(1)} | target=${m.targetCaseId} | alt=${m.alternativeCaseId ?? "none"}`);
        log(`    emails: ${m.emailIds.length} | subject: ${email?.subject?.slice(0, 60) ?? "?"}`);
        if (m.breakdown) {
          log(`    breakdown: thread=${m.breakdown.threadScore} subj=${m.breakdown.subjectScore.toFixed(1)} tag=${m.breakdown.tagScore.toFixed(1)} actor=${m.breakdown.actorScore} decay=${m.breakdown.timeDecayMultiplier.toFixed(3)}`);
        }
      }
    }
    ...
    for (const c of creates.slice(0, 10)) {
      ...
      log(`  syntheticId=${c.entityId ? "has-entity" : "no-entity"} | emails=${c.emailIds.length} | subject: ${email?.subject?.slice(0, 60) ?? "?"}`);
    }
    ...
    log(`  Emails accounted for:         ${decisions.reduce((s, d) => s + d.emailIds.length, 0)} of ${emailInputs.length}`);
    ...
    if (nearThreshold.length > 0) {
      ...
      for (const d of nearThreshold.slice(0, 10)) {
        ...
        log(`  action=${d.action} score=${d.score.toFixed(1)} | ${email?.subject?.slice(0, 50) ?? "?"}`);
        if (d.breakdown) {
          log(`    subj=${d.breakdown.subjectScore.toFixed(1)} tag=${d.breakdown.tagScore.toFixed(1)} actor=${d.breakdown.actorScore} decay=${d.breakdown.timeDecayMultiplier.toFixed(3)}`);
        }
      }
    }
  }
...
main().catch((e) => { log("FAIL: " + e.message + "\n" + e.stack); process.exit(1); });
