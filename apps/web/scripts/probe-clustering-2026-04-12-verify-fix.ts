...
async function main() {
  ...
  log(
    `Total emails accounted for: ${emailCounts.reduce((s, n) => s + n, 0)} of ${emailInputs.length}`,
  );
  const singletons = emailCounts.filter((c) => c === 1).length;
  log(
    `Singleton cases: ${singletons} of ${caseEmails.size} (${((singletons / caseEmails.size) * 100).toFixed(0)}%)`,
  );
