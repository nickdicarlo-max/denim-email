							...
							>
								...
								{routingDecision && (
									<div className="mt-1 ml-12 text-[11px] text-muted">
										{routingDecision.method && (
											<span>route: {routingDecision.method}</span>
										)}
										{routingDecision.relevanceScore != null && (
											<span className="ml-2">relevance: {routingDecision.relevanceScore.toFixed(2)}</span>
										)}
								...
								{breakdown && cluster?.action === "MERGE" && (
									<div className="mt-1 ml-12 font-mono text-[10px] text-muted flex flex-wrap gap-x-3">
										...
										{breakdown.timeDecayMultiplier != null && <span>decay:{breakdown.timeDecayMultiplier.toFixed(2)}</span>}
									</div>
								)}
