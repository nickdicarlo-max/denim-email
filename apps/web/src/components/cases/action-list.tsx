...
export function ActionList({ actions, schemaId }: ActionListProps) {
	...
	const toggleAction = useCallback(async (actionId: string, currentStatus: string) => {
		...
		// Optimistic update
		setItems((prev) => prev.map((a) => a.id === actionId ? { ...a, status: newStatus as Action["status"] } : a));
		...
		try {
			...
			if (!res.ok) {
				// Revert on failure
				setItems((prev) => prev.map((a) => a.id === actionId ? { ...a, status: currentStatus as Action["status"] } : a));
			}
		} catch {
			// Revert on error
			setItems((prev) => prev.map((a) => a.id === actionId ? { ...a, status: currentStatus as Action["status"] } : a));
...
function ActionItem({ action, onToggle }: { action: Action; onToggle: (id: string, status: string) => void }) {
	...
	return (
		<div className={`flex items-start gap-2 text-sm ${style.classes}`}>
			<button
				...
				title={isToggleable ? (action.status === "PENDING" ? "Mark done" : "Mark pending") : undefined}
			>
				{style.icon}
			</button>
