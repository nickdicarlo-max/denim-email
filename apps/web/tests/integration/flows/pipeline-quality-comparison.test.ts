...
describe.skipIf(!HAS_GMAIL_TOKEN)("Pipeline Quality Comparison: Old vs New Pipeline", () => {
  ...
  beforeAll(async () => {
    ...
    console.log(
      `Schema 2 verified: ${entities.length} entities, ${tags.length} tags, ${groups.length} groups`,
    );
  }, 120_000);
  ...
  it("runs new pipeline and compares with old pipeline", async () => {
    ...
    console.log(`Extraction: processed=${batchResult.processed}, excluded=${batchResult.excluded}`);
    ...
    if (entityDist.length < 2) {
      console.warn(
        `  WARNING: Only ${entityDist.length} entity used — discovery may have been skewed by email cap`,
      );
    } else {
    ...
    try {
      ...
      const s2Relevant = await prisma.email.count({
        where: { schemaId: schema2Id, isExcluded: false },
      });
      ...
      if (schema1Id) {
        ...
        s1Relevant = await prisma.email.count({
          where: { schemaId: schema1Id, isExcluded: false },
        });
      ...
      console.log(
        row("Excluded (relevance)", schema1Id ? s1RelevanceExcluded : "N/A", s2RelevanceExcluded),
      );
