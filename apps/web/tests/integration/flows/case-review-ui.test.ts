...
describe("Case Review UI (HTTP)", () => {
  ...
  it("filters cases by status", async () => {
    const res = await withTimeout(
      api.get(`/api/cases?schemaId=${testSchema.schema.id}&status=OPEN`),
      30_000,
  ...
  it("filters cases by entityId", async () => {
    const res = await withTimeout(
      api.get(`/api/cases?schemaId=${testSchema.schema.id}&entityId=${testSchema.entities.vms.id}`),
      30_000,
  ...
  it("rejects invalid limit with 400", async () => {
    const res = await withTimeout(
      api.get(`/api/cases?schemaId=${testSchema.schema.id}&limit=999`),
      30_000,
  ...
  it("rejects missing schemaId with 400", async () => {
    const res = await withTimeout(api.get("/api/cases"), 30_000, "GET /api/cases (no schemaId)");
  ...
  it("returns full case detail with emails and actions", async () => {
    ...
    const res = await withTimeout(api.get(`/api/cases/${caseId}`), 30_000, "GET /api/cases/[id]");
  ...
  it("updates viewedAt on case detail view", async () => {
    ...
    // View the case
    await withTimeout(api.get(`/api/cases/${caseId}`), 30_000, "GET /api/cases/[id] (viewedAt)");
  ...
  it("returns schema summary with status counts", async () => {
    ...
    const total = Object.values(body.data.statusCounts).reduce((a, b) => a + b, 0);
    expect(total).toBe(3);
  ...
  it("thumbs down with reason creates FeedbackEvent", async () => {
    ...
    expect((event!.payload as Record<string, unknown>).reason).toBe("wrong_group");
  });
  ...
  it("cursor pagination returns different results", async () => {
    ...
    const page1 = await withTimeout(
      api.get(`/api/cases?schemaId=${testSchema.schema.id}&limit=1`),
      30_000,
