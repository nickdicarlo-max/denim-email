...
describe("Pipeline Wiring: ScanJob phase transitions + clustering → synthesis", () => {
  ...
  beforeAll(async () => {
    ...
    // seedTestEmails uses createMany which doesn't trigger increment hooks,
    // so manually set emailCount to match the 6 non-excluded emails
  ...
  it(
    "runs clustering → synthesis with ScanJob phase transitions",
    async () => {
      ...
      // Update ScanJob with cluster results
      await prisma.scanJob.update({
        where: { id: scanJobId },
        data: {
          casesCreated: clusterResult.casesCreated,
          casesMerged: clusterResult.casesMerged,
      ...
      // Verify ScanJob phase and counts
      const afterClustering = await prisma.scanJob.findUniqueOrThrow({
        where: { id: scanJobId },
      });
      expect(afterClustering.phase).toBe("CLUSTERING");
      expect(afterClustering.casesCreated).toBe(clusterResult.casesCreated);
      expect(afterClustering.clustersCreated).toBe(clusterResult.clustersCreated);
      ...
      expect(finalJob.casesCreated).toBeGreaterThan(0);
      ...
      console.log(
        `Direct wiring test: ${cases.length} cases synthesized, ScanJob COMPLETED`,
      );
    },
    300_000,
  );
...
describe.skipIf(!HAS_INNGEST)(
  "Inngest Event Chain: extraction.all.completed → COMPLETED",
  () => {
    let scanJobId: string;
    let schemaId: string;
    ...
    beforeAll(async () => {
      ...
      // Manually set emailCount
      await prisma.caseSchema.update({
        where: { id: schemaId },
        data: { emailCount: 6 },
      });

      // Create ScanJob at EXTRACTING (as if extraction just finished)
      const scanJob = await prisma.scanJob.create({
        data: {
          schemaId,
    ...
    it(
      "emitting extraction.all.completed triggers the full clustering → synthesis chain",
      async () => {
        ...
        console.log(
          `Emitted extraction.all.completed for schema=${schemaId}, scanJob=${scanJobId}`,
        );
        ...
        while (Date.now() - startTime < MAX_WAIT_MS) {
          ...
          if (job.status === "FAILED") {
            throw new Error(
              `ScanJob failed: ${job.statusMessage ?? "unknown error"}`,
            );
          }
        ...
        console.log(
          `Inngest chain test: ${cases.length} cases synthesized via event chain`,
        );
      },
      360_000,
    );
