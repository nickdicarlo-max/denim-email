...
async function main() {
  ...
  console.log('Users:', users.map(u => ({ id: u.id, email: u.email, hasTokens: !!u.googleTokens })));

  // Check the scanJob's userId matches
  if (j) {
    console.log('ScanJob userId:', j.userId);
    const user = await p.user.findUnique({ where: { id: j.userId }, select: { id: true, googleTokens: true } });
