...
**Working on `feature/ux-overhaul` (refactor is intentionally coupled to the UX work).** As of 2026-04-08, **16 of 18 tasks are landed. Phases 0–9 are complete** — server state machine, HTTP routes, client flow collapse, and dead-code deletion are all done. The remaining two tasks are e2e tests and cron + final verification.
...
- Phase 9 (complete) — dead code deleted. Task 15 removed the 8 files from the plan's delete list: the 4 interview routes (`hypothesis`, `finalize`, `validate`, `review-finalize`), `api/schemas/[schemaId]/status/route.ts`, the `onboarding/scanning` and `onboarding/review` pages, and `components/onboarding/scan-stream.tsx`, plus the empty parent dirs. Also deleted the standalone `interview.test.ts` (all 9 tests were HTTP against the deleted routes) and trimmed the HTTP-Finalize describe block from `entity-groups.test.ts` (17→13 tests). Updated the JSDoc example in `tests/integration/helpers/timeout.ts`. Typecheck clean first try, all remaining integration suites green (entity-groups 13, onboarding-polling 20, onboarding-state 18, scan-metrics 8) and package units 126/126. Net diff: −1429 / +127, about 1300 lines removed.

**Remaining (2 tasks, tests + final verification):**
...
**Next:** Phase 10 / Task 16 — `apps/web/tests/integration/onboarding-happy-path.test.ts` against a real Inngest dev server on `:8288` and a real Next dev server on `:3000`. Scenarios from the plan: (1) happy path end-to-end from `POST /api/onboarding/start` through `POST /api/onboarding/:schemaId` review confirm, verifying phase progression and the final `status=ACTIVE`; (2) failure path — inject a mid-scan error, verify the schema lands in `phase=FAILED`, hit the retry route, verify recovery for a pre-scan failure; (3) re-entry path — send `onboarding.session.started` twice for the same schemaId and verify the idempotency guard doesn't double-run the workflow. Plan also mentions a `scan-accounting.test.ts` and several other scenario files but the critical-path ones are happy-path + resume + concurrent-start.
