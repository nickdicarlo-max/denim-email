...
**Working on `feature/ux-overhaul` (refactor is intentionally coupled to the UX work).** As of 2026-04-08, **17 of 18 tasks are landed. Phases 0–10 are complete** — server state machine, HTTP routes, client flow collapse, dead-code deletion, and e2e test suite are all done. The remaining task is Phase 11: cron stub (Task 17) + final verification (Task 18).
...
- Phase 10 (complete) — e2e test suite. Task 16 landed four new test files totalling ~1000 lines: (a) `onboarding-scan-accounting.test.ts` — 6 pure-Prisma tests pinning the `processed+excluded+failed==total` invariant, no dev server needed, 6/6 green locally; (b) `onboarding-concurrent-start.test.ts` — 4 tests verifying Task 10 idempotency under concurrent POSTs (requires Next dev server); (c) `onboarding-routes.test.ts` — ~16 HTTP contract tests for Tasks 11, 12, 13 (polling, confirm, cancel, retry, scan list/create/detail — requires Next dev server); (d) `onboarding-happy-path.test.ts` — full Inngest + live AI e2e, gated on `RUN_E2E_HAPPY=1`, points at the OAuth playground + 433-email fixture options for wiring up Gmail access. Plan deviation: all tests use the real `createTestUser()` + Bearer flow instead of the plan's fake `x-test-user-id` auth-bypass header (which `withAuth` doesn't honor).

**Remaining (1 task, cron + final verification):**
...
**Next:** Phase 11 — Task 17 + Task 18 (both small). Task 17 is a cron stub at `apps/web/src/lib/inngest/cron.ts` that emits `scan.requested` for every `status=ACTIVE` schema whose `lastScannedAt` is older than a threshold. Plan wires it to an Inngest event trigger (`cron.daily.scans.trigger`) rather than a real cron schedule so it can be manually kicked off until the team is ready to enable it in production. Task 18 is the final verification sweep: full typecheck, all package units (126) + integration tests, optional eval rerun, then flip this doc's "In Progress" section to "Done — 2026-04-08" with the eval resolutions listed (#14 idempotency, #15 polling, #16 accounting, #17 flat shape, #18 schemaId-in-URL). After Task 18 the feature/ux-overhaul branch is merge-ready.
