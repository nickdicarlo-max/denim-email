...
| 10 | 16. End-to-end integration tests | ✅ done | `eba7dac` | **Scoped pragmatically** — 4 test files instead of the plan's 5, covering structural guarantees without requiring live AI / Gmail / Inngest: `onboarding-scan-accounting.test.ts` (6 tests, pure Prisma — pins the `processed+excluded+failed==total` invariant), `onboarding-concurrent-start.test.ts` (4 tests, requires Next :3000 — Task 10 idempotency under race), `onboarding-routes.test.ts` (~16 tests, requires Next :3000 — HTTP contracts for Tasks 11/12/13), `onboarding-happy-path.test.ts` (1 test, gated on `RUN_E2E_HAPPY=1` — full e2e with Inngest + live AI + Gmail fixture). **Deviation:** plan's sketches used a fake `x-test-user-id` bypass header that `withAuth` doesn't honor — all tests use the real `createTestUser()` + Bearer flow. Scan-accounting 6/6 green locally; HTTP-dependent tests validated by user when dev server is running. |
