...
**Branch:** `feature/ux-overhaul` · **Status:** ✅ **REFACTOR COMPLETE (2026-04-08)** · 18 of 18 tasks shipped across Phases 0–11. Final verification sweep (Task 18) green: typecheck clean, 126/126 package units, 87/87 non-AI integration tests. Branch is merge-ready. Follow-ups captured as issue #33 (outbox-pattern structural fix for the fragile TOCTOU P2002 pattern + Inngest-outage stranding) and the scan-stage retry limitation (deferred, see debt list below).
...
| 11 | 17. Cron stub | ✅ done | `9a1f9e0` | **Went beyond the plan's stub** — not just a manual-trigger wiring test. Discovered that `lastScannedAt` was a dead column (never written), so Task 17 also added a `stamp-last-scanned-at` step to `runSynthesis` and wired the cron's filter to use it (`lastScannedAt IS NULL OR < now - 23h`, ordered ASC NULLS FIRST). New `cron.daily.scans.trigger` event in `DenimEvents`. Per-schema last-mile conflict check for already-active ScanJobs. Kept the event trigger (not real cron) for v1 so the team can validate the wiring against dev before enabling a schedule. Deferred: per-schema `scanFrequency` handling, timezone awareness, function-level retries. |
| 11 | 18. Full verification + status doc update | ✅ done | (this commit) | Final verification sweep: apps/web typecheck clean, packages 126/126 (types 2 + engine 92 + ai 32), integration suite 87/87 (scan-accounting 6 + scan-metrics 8 + onboarding-state 18 + onboarding-polling 20 + entity-groups 13 + concurrent-start 4 + routes 18). **Full count: 213/213 green.** Gated tests intentionally not run: onboarding-happy-path (RUN_E2E_HAPPY=1), extraction (live Gemini), inngest-pipeline Test 2B. Canonical status doc flipped from "In Progress" to "Done — 2026-04-08" with the five eval-issue resolutions (#14–#18) listed explicitly. Branch merge-ready. |
...
- ~~**Inngest-outage stranding in `POST /api/onboarding/start`**~~ — **resolved by #33** (2026-04-09). Transactional outbox pattern (Option C from #33): `{CaseSchema stub, OnboardingOutbox row}` written in one Prisma `$transaction`; `drainOnboardingOutbox` cron (every 1 min, exponential backoff, dead-letter at 10 attempts) retries failed emissions. The fragile TOCTOU P2002-catch is also eliminated — `onboarding_outbox.schemaId` PK is the sole idempotency guard. New integration test `onboarding-outbox-stranding.test.ts` covers both happy drain and backoff paths.
