...
function detectDomain(text) {
  ...
  if (t.includes("school") || t.includes("kid") || t.includes("child") || t.includes("parent") || t.includes("soccer") || t.includes("sports")) return "school_parent";
  if (t.includes("rental") || t.includes("property") || t.includes("tenant") || t.includes("landlord") || t.includes("unit")) return "property";
  if (t.includes("construction") || t.includes("renovation") || t.includes("contractor") || t.includes("project manager")) return "construction";
  if (t.includes("law") || t.includes("attorney") || t.includes("legal") || t.includes("client")) return "legal";
  if (t.includes("agency") || t.includes("marketing") || t.includes("client") || t.includes("campaign")) return "agency";
  return "general";
}

const FOLLOW_UPS = {
  school_parent: {
    q1: { question: "How do you mainly think about organizing this?", options: ["By each child", "By each activity", "All together"] },
    q2: { question: "Should I track the people involved (teachers, coaches) for each?", options: ["Yes, definitely", "Not important"] },
  },
  property: {
    q1: { question: "How do you mainly organize your work?", options: ["By property", "By vendor", "By issue type"] },
    q2: { question: "Do you want to track costs from invoices and quotes?", options: ["Yes, track costs", "Not needed"] },
  },
  construction: {
    q1: { question: "How do you organize your projects?", options: ["By job site / project", "By subcontractor", "By phase"] },
    q2: { question: "Should I track costs and deadlines from emails?", options: ["Yes, both", "Just deadlines", "Not needed"] },
  },
  legal: {
    q1: { question: "How do you organize your work?", options: ["By client / matter", "By case type", "By court / jurisdiction"] },
    q2: { question: "Should I track deadlines and filing dates?", options: ["Yes, critical", "Nice to have"] },
  },
  agency: {
    q1: { question: "How do you organize your projects?", options: ["By client", "By campaign", "By deliverable type"] },
    q2: { question: "Should I track deadlines and deliverable status?", options: ["Yes, track both", "Just deadlines"] },
  },
  general: {
    q1: { question: "What's the main way you'd group these emails?", options: ["By project / topic", "By person", "By time period"] },
    q2: { question: "Is there specific information you want pulled from emails?", options: ["Dates and deadlines", "Costs and amounts", "Action items", "Not sure yet"] },
  },
};
...
const HYPOTHESES = {
  school_parent: {
    ...
    primaryEntity: { name: "Activity", description: "A school or sports activity your child participates in" },
    entities: [
      { name: "Vail Mountain School", type: "primary", confidence: 0.95, source: "description" },
      { name: "Eagle Valley Soccer Club", type: "primary", confidence: 0.92, source: "description" },
    ],
    secondaryTypes: [
    ...
    tags: [
      { name: "Action Required", freq: "high", actionable: true, desc: "Parent needs to do something" },
      { name: "Schedule", freq: "high", actionable: true, desc: "Date, time, or calendar event" },
      { name: "Payment", freq: "medium", actionable: true, desc: "Fee, registration, or fundraiser cost" },
      { name: "Permission / Form", freq: "medium", actionable: true, desc: "Document requiring signature" },
      { name: "Game / Match", freq: "medium", actionable: true, desc: "Athletic competition with date/location" },
      { name: "Practice", freq: "high", actionable: false, desc: "Regular training session" },
      { name: "Cancellation", freq: "low", actionable: true, desc: "Event cancelled or rescheduled" },
    ...
    extractedFields: [
      { name: "eventDate", type: "date", desc: "Date/time of events, practices, games", showOnCard: true },
      { name: "eventLocation", type: "string", desc: "Field, gym, school location", showOnCard: false },
    ...
    clusteringHighlights: {
      ...
      rationale: "School events are planned months ahead with frequent reminders. Looser clustering and long time horizons.",
    },
  },
  property: {
    domain: "property",
    ...
    extractedFields: [
      { name: "cost", type: "number", desc: "Dollar amount from invoices/quotes", showOnCard: true },
...
// --- ICONS ---
const Icons = {
  mail: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 4L12 13L2 4"/></svg>,
  check: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M5 12L10 17L19 7"/></svg>,
  x: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 6L6 18M6 6L18 18"/></svg>,
  ...
  search: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><path d="M21 21L16.65 16.65"/></svg>,
  arrow: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12H19M12 5L19 12L12 19"/></svg>,
  back: <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 4L6 10L12 16"/></svg>,
};
    ...
    <div style={{
      ...
      <div>
        ...
        <div style={{ fontSize: "11px", color: T.textMuted, marginTop: "1px" }}>
          {entity.source === "description" ? "From your description" : entity.source === "user_added" ? "Added by you" : "Found in email"}
          {entity.confidence < 0.9 && entity.source !== "user_added" && ` (${Math.round(entity.confidence * 100)}% match)`}
        </div>
      </div>
      <div style={{ display: "flex", gap: "8px" }}>
        {onEdit && <span onClick={onEdit} style={{ cursor: "pointer", color: T.textMuted, display: "flex" }}>{Icons.edit}</span>}
        {onRemove && <span onClick={onRemove} style={{ cursor: "pointer", color: T.textMuted, display: "flex" }}>{Icons.x}</span>}
      </div>
    </div>
  );
}
    ...
    <div style={{ padding: "24px 20px", display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ flex: 1, overflow: "auto" }}>
        ...
        <div style={{ marginBottom: "16px", marginTop: "12px" }}>
          <h2 style={{ fontSize: "20px", fontWeight: 700, color: T.text, margin: "0 0 6px", letterSpacing: "-0.02em", lineHeight: 1.2 }}>
        ...
        {step === 1 ? (
          ...
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            ...
            <div
              onClick={() => { setStep(1); setRole(null); setWhats([]); setWhos([]); setCurrentWhat(""); setCurrentWho(""); setShowWho(false); setGoals([]); }}
              style={{
                padding: "8px 12px", borderRadius: T.radiusSm,
                background: T.accentSoft, display: "inline-flex",
                alignItems: "center", gap: "8px", fontSize: "13px",
                color: T.accentText, fontWeight: 500, marginBottom: "16px",
            ...
            >
              <span style={{ display: "flex", color: T.accentText, opacity: 0.5 }}>{Icons.back}</span>
            ...
            {dc && (
              <>
                <div style={{
                  fontSize: "11px", fontWeight: 600, color: T.accentText,
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "8px" }}>
                    {whats.map((name, i) => (
                      <span key={i} style={{
                        ...
                        {name}
                        <span onClick={() => handleRemoveWhat(i)} style={{ cursor: "pointer", display: "flex", opacity: 0.6 }}>{Icons.x}</span>
                      </span>
                    ))}
                  </div>
                )}
                ...
                <div style={{ display: "flex", gap: "6px", marginBottom: "16px" }}>
                  <input
                    ...
                    onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); handleAddWhat(); } }}
                    ...
                    style={{
                      ...
                      fontSize: "14px", fontFamily: T.font, color: T.text,
                  ...
                  <button
                    ...
                    style={{
                      padding: "11px 16px", borderRadius: T.radiusSm, border: "none",
                ...
                {whats.length > 0 && (
                  <>
                    {!showWho ? (
                      <div
                        onClick={() => { setShowWho(true); setTimeout(() => whoRef.current?.focus(), 100); }}
                        style={{
                          display: "flex", alignItems: "center", gap: "6px",
                          padding: "10px 14px", borderRadius: T.radiusSm,
                          border: `1.5px dashed ${T.border}`, background: "transparent",
                          cursor: "pointer", fontSize: "13px", fontWeight: 500,
                          color: T.textSecondary, fontFamily: T.font, width: "100%",
                        }}
                        onMouseEnter={e => { e.currentTarget.style.borderColor = T.warn; e.currentTarget.style.color = T.warnText; }}
                      ...
                      <div style={{ animation: "fadeIn 0.2s ease" }}>
                        <div style={{
                          fontSize: "11px", fontWeight: 600, color: T.warnText,
                          textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "4px",
                        }}>
                          {dc.whoLabel}
                        </div>
                          ...
                          <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "8px" }}>
                            {whos.map((name, i) => (
                              <span key={i} style={{
                                ...
                                {name}
                                <span onClick={() => handleRemoveWho(i)} style={{ cursor: "pointer", display: "flex", opacity: 0.6 }}>{Icons.x}</span>
                              </span>
                            ))}
                          </div>
                        )}
                        ...
                        <div style={{ display: "flex", gap: "6px" }}>
                          <input
                            ...
                            onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); handleAddWho(); } }}
                    ...
                    {/* Reassurance */}
                    <div style={{
                      marginTop: "14px", padding: "10px 12px", borderRadius: T.radiusSm,
                      background: T.bgAccent, fontSize: "12px", color: T.textMuted,
                      lineHeight: 1.4, display: "flex", alignItems: "flex-start", gap: "8px",
                    }}>
                      <span style={{ display: "flex", flexShrink: 0, marginTop: "1px", color: T.accent }}>{Icons.sparkle}</span>
                    ...
                    {dc.goals && (
                      <div style={{ marginTop: "16px" }}>
                        <div style={{
                          fontSize: "11px", fontWeight: 600, color: T.successText,
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                          {dc.goals.map(g => {
                            ...
                            return (
                              <button
                                key={g.id}
                                onClick={() => setGoals(prev => selected ? prev.filter(x => x !== g.id) : [...prev, g.id])}
                                style={{
                                  padding: "7px 12px",
                                  borderRadius: "100px",
      ...
      {canContinue && (
        <button
          ...
          style={{
            marginTop: "12px", padding: "14px", borderRadius: T.radiusSm,
            border: "none", background: T.accent, color: T.textInverse,
            fontSize: "15px", fontWeight: 600, cursor: "pointer",
            fontFamily: T.font, display: "flex", alignItems: "center",
            justifyContent: "center", gap: "8px", animation: "fadeIn 0.3s ease",
          }}
        >
...
function Card2({ onNext }) {
  ...
  return (
    <div style={{ padding: "24px 20px", display: "flex", flexDirection: "column", height: "100%", justifyContent: "center", alignItems: "center" }}>
      <ProgressDots current={1} total={4} />

      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", maxWidth: "320px" }}>
        <div style={{
          width: "64px",
          height: "64px",
          borderRadius: "50%",
        ...
        <h2 style={{ fontSize: "22px", fontWeight: 700, color: T.text, margin: "0 0 8px", letterSpacing: "-0.02em" }}>
          {connected ? "Connected!" : "Connect your email"}
        </h2>
        <p style={{ fontSize: "14px", color: T.textSecondary, margin: "0 0 24px", lineHeight: 1.5 }}>
          {connected
    ...
    <div style={{ padding: "24px 20px", display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
          ...
          <span style={{ fontSize: "12px", fontWeight: 600, color: T.text, fontFamily: T.fontMono }}>
      ...
      {/* Discoveries */}
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: "11px", fontWeight: 600, color: T.textMuted, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "10px" }}>
...
function Card4({ domain, card1Data, onFinalize }) {
  ...
  const [entities, setEntities] = useState(hypothesis.entities.map(e => ({ ...e, active: true })));
  ...
  const handleAddTag = () => {
    if (newTagName.trim()) {
      setTags(prev => [...prev, { name: newTagName.trim(), freq: "medium", actionable: false, desc: "Custom tag", active: true }]);
      setNewTagName("");
      setAddingTag(false);
    }
  };
  ...
  if (confirmed) {
    return (
      <div style={{ padding: "24px 20px", display: "flex", flexDirection: "column", height: "100%", justifyContent: "center", alignItems: "center", textAlign: "center" }}>
        <div style={{
          width: "72px", height: "72px", borderRadius: "50%",
          background: T.successSoft, display: "flex", alignItems: "center", justifyContent: "center",
          marginBottom: "20px", color: T.success,
        }}>
          {Icons.check}
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: T.bgAccent, overflow: "hidden",
          }}>
            <div style={{
          ...
          <div style={{ fontSize: "12px", color: T.textMuted }}>
            {entities.filter(e => e.active).length} topics / {tags.filter(t => t.active).length} categories
          </div>
        </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ padding: "16px 20px 0" }}>
        ...
        <h2 style={{ fontSize: "20px", fontWeight: 700, color: T.text, margin: "12px 0 4px", letterSpacing: "-0.02em" }}>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "0 20px" }}>
            ...
            {!addingEntity ? (
              ...
              <div style={{
                ...
                <input
                  ...
                  onKeyDown={e => { if (e.key === "Enter") handleAddEntity(); if (e.key === "Escape") { setAddingEntity(false); setNewEntityName(""); } }}
                ...
                <button
                  onClick={() => { setAddingEntity(false); setNewEntityName(""); }}
                  style={{
                    padding: "6px",
                    borderRadius: T.radiusXs,
                    border: "none",
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
            ...
            {!addingTag ? (
              ...
              <div style={{ display: "flex", gap: "4px", alignItems: "center" }}>
                ...
                />
                <span onClick={handleAddTag} style={{ cursor: "pointer", color: T.accent, display: "flex" }}>{Icons.check}</span>
                <span onClick={() => { setAddingTag(false); setNewTagName(""); }} style={{ cursor: "pointer", color: T.textMuted, display: "flex" }}>{Icons.x}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          {hypothesis.extractedFields.map((f, i) => (
            <div key={i} style={{
              ...
              alignItems: "center",
              padding: "8px 0",
              borderBottom: i < hypothesis.extractedFields.length - 1 ? `1px solid ${T.borderLight}` : "none",
            }}>
              <div>
                <div style={{ fontSize: "13px", fontWeight: 500, color: T.text }}>{f.desc}</div>
                <div style={{ fontSize: "11px", color: T.textMuted }}>{f.name} ({f.type})</div>
              </div>
              {f.showOnCard && (
                <span style={{
                  fontSize: "10px",
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", fontFamily: T.fontMono, fontSize: "11px" }}>
              ...
              <span>Reminders: {hypothesis.clusteringHighlights.reminderCollapse ? "on" : "off"}</span>
            </div>
          </div>
        </div>
